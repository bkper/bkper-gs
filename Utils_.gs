var Utils_ = {
  ...
  getDiacriticsMap_: function() {
    if (Utils_.diacriticsMap_ == null) {
      Utils_.diacriticsMap_ = {};
      for (var i=0; i < Utils_.defaultDiacriticsRemovalap_.length; i++){
        var letters = Utils_.defaultDiacriticsRemovalap_[i].letters.split("");
        for (var j=0; j < letters.length ; j++){
          Utils_.diacriticsMap_[letters[j]] = Utils_.defaultDiacriticsRemovalap_[i].base;
        }
      }    
    }
    return Utils_.diacriticsMap_;
  },
  ...
  removeDiacritics: function(str) {
    ...
    for(var i=0; i< letters.length; i++) {
      newStr += letters[i] in Utils_.getDiacriticsMap_() ? Utils_.getDiacriticsMap_()[letters[i]] : letters[i];
    }
    return newStr;
  },
