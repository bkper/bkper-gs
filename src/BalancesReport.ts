...
class BalancesReport {
  ...
  public getBalancesContainer(name: string): BalancesContainer {
    ...
    let balancesContainersNames: string[] = this.getBalancesContainers().map(b => b.getName())
    throw `${name} not found among ${balancesContainersNames} on book [${this.book.getName()}].`;
  }
