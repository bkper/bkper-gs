namespace GroupService_ {
    ...
    export function createGroup(bookId: string, group: bkper.Group): bkper.Group {
        ...
        var responseJSON = new HttpBooksApiV5Request(`${bookId}/groups`).setMethod('post').setPayload(payload).fetch().getContentText();
        return JSON.parse(responseJSON);
    }

    export function updateGroup(bookId: string, group: bkper.Group): bkper.Group {
        var payload = JSON.stringify(group);
        var responseJSON = new HttpBooksApiV5Request(`${bookId}/groups`).setMethod('put').setPayload(payload).fetch().getContentText();
        return JSON.parse(responseJSON);
    }

    export function deleteGroup(bookId: string, group: bkper.Group): bkper.Group {
        var responseJSON = new HttpBooksApiV5Request(`${bookId}/groups/${group.id}`).setMethod('delete').fetch().getContentText();
        return JSON.parse(responseJSON);
    }
    ...
    export function createGroups(bookId: string, groups: bkper.Group[]): bkper.Group[] {
        ...
        var responseJSON = new HttpBooksApiV5Request(`${bookId}/groups/batch`).setMethod('post').setPayload(groupsBatchJSON).fetch().getContentText();
